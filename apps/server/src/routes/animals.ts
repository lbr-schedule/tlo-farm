...
router.post('/chicken-coop/feed-all', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const invItem = invRows.rows[0];
    const feedAmount = invItem?.amount ?? 0;

    if (feedAmount < slots.length) {
      return res.status(400).json({ success: false, message: `普通飼料不足！需要 ${slots.length} 包，背包有 ${feedAmount} 包` });
    }
...
router.post('/chicken-coop/collect', async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Add egg to inventory (livestock type, item_id = 1)
    const eggInvRows = await db.execute(
      `SELECT id, amount FROM inventories WHERE user_id = ? AND item_type = 'livestock' AND item_id = 1`,
      [userId]
    );
    ...
    if (existingEgg) {
      ...
      await db.execute(
        `INSERT INTO inventories (user_id, item_type, item_id, amount) VALUES (?, 'livestock', 1, 1)`,
        [userId]
      );
    }
...
router.post('/chicken-coop/collect-all', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (typeof eggCount !== 'number' || eggCount <= 0) {
      return res.status(400).json({ success: false, message: '沒有雞蛋可收取' });
    }

    // 找出所有 READY_TO_COLLECT 的槽位
    const slotRows = await db.execute(
      `SELECT id, slot_index FROM chicken_slots WHERE user_id = ? AND state = 'READY_TO_COLLECT'`,
      [userId]
    );
